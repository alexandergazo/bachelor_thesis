...
\begin{definition}
	Let $\K$ be a field and let $A \in \K^{n \times n}$, $B \in \K^{n \times m}$, $n,m \in \N$. The pair $(A,B)$ is \termdef{controllable} or \termdef{reachable} if $\textnormal{dim}\mathcal{R}(A,B)=n$.
\end{definition}
...
We will need two properties of this notion which we will form in the next claim. 
...
\begin{proof}
	1. We have $$\frac{d}{dt}e^{Xt}=\frac{d}{dt}\sum^\infty_{k=0}\frac{1}{k!}(Xt)^{k}=\frac{d}{dt}\sum^\infty_{k=0}\frac{t^k}{k!}X^{k}=\sum^\infty_{k=0}\frac{t^k}{k!}X^{k+1}=X\sum^\infty_{k=0}\frac{t^k}{k!}X^{k}=Xe^{Xt}$$
	2. From definition follows $$e^{At}B=\sum^\infty_{k=0}\frac{t^k}{k!}A^{k}B\stackrel{AB=BA}{=}\sum^\infty_{k=0}\frac{t^k}{k!}BA^{k}=B\sum^\infty_{k=0}\frac{t^k}{k!}A^{k}=Be^{At}$$ 
\end{proof}
...
Now it is clear that in system where $x(0)=\nullvector$ can every state in time $t\in \R^+$ be express as $$x(t)=\int^t_0 e^{(t-s)A}Bu(s)ds$$
...
\begin{proof}
	If the dimension condition fails we know that there exists vector $\tilde{v}\in\K^n$ which cannot be expressed as linear combination of any basis of $\mathcal{R}(A,B)$. The vector $\tilde{v}$ can then be orthogonalized using Gram-Schmidt orthogonalization. Let us denote the orthogonalized vector by $v$. Then for arbitrary vector $u\in\mathcal{R}(A,B)$ we have $v\cdot u=v^Tu=0$. Consequently $v^TA^iB=\nullvector$ for $i\in\N_0$.
\end{proof}
